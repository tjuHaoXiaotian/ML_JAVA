...
//            System.out.println(this.getId()+ " cal message differ : " + scoreNew + " - " + scoreOld + " = " + differ);
    ...
    public int getMaxAction(int expId) {
        if(expId == 0 || expId == 1){   // dcop
            return maxAction;
        }else{
            return getMaxSelfAction();  // single q or with neighbors
        }
    }
     ...
     */
    private int selectActionWithExploration(int expId){
        if(Math.random() < exploreRate){  // 随机探索
            return Config.getRandomNumber(0,actionNum-1);
        }else{
            if(expId == 0 || expId ==1){  // dcop
                return maxAction;
            }else if(expId == 2){ // single
                return getMaxSelfAction();
            }else {  //expId == 3 neighbors
                return getMaxWithNeighbors();
            }
        }
    }

    private int getMaxWithNeighbors(){
        ...
        for(Node node:getNeighbors()){
            // 1.1：统计通信次数信息
            Analyze.incCommunicationTimes(type,3);
            neighbor = (Agent)node;
            for(int i = 0; i < neighbor.actionNum;i++){
                actionsReward[i] += neighbor.q[i];
            }
        }
    ...
    public int getMaxSelfAction(){
        // 2：选择表现最好的 action
        int action = Config.getRandomNumber(0,actionNum-1);
        double maxValue = q[action];
        int maxAction = action;
...
//        partnerQTable.getQ()[partnerAction][action] = (1 - partner.learningRate) * partnerQTable.getQ()[partnerAction][action] + partner.learningRate * (reward + Config.discountParameter * partnerQTable.getQ()[globalActionPartner][globalActionMe]) ;
     ...
     * @param reward
     */
    private void updateQSelf(int action,double reward){

        // 1): 更新各自 Q table
        q[action] = (1 - learningRate) * q[action] + learningRate * reward;
    }

    private void printQ(int expId,int partnerId){
        if(expId == 0 || expId == 1){
            ...
            System.out.println("Q["+this.getId()+"] ============================================================");
            for(double qitem:q){
    ...
    /**
     * 每一次训练
     * expId 0: run DCOP,adjuest the coordination set (loss rate 0.1)
     * expId 1: run DCOP,adjuest the coordination set, loss rate 0.8
     * expId 2: independent learners
     * expId 3: distributed value functions
     * @param expId
     */
    public void training(int expId){
        //       learning============================================================================

        if(expId == 0 || expId == 1){  // run DCOP, with different loss rate
            // 1:随机选择一个agent，交互
            ...
            int action = selectActionWithExploration(expId);
            int partnerAction = partner.selectActionWithExploration(expId);
