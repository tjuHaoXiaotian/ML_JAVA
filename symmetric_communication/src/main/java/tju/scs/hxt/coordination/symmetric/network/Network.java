...
public class Network {
    ...
    public static ArrayList<Agent> generateRandomRegularGraph(int nodesNum,int neighborNum,int type){
        ...
        // 3：将其余的边随机连接起来
        ArrayList<Integer> excepts = null;
        ...
        for(int i = 0; i < nodesNum;i++){
            for(int j = 1; j <= neighborNum;j++){
                if(Math.random() < p){  // 概率范围之内，需要重连
                    ...
                    while (me.equals(relinkNeighbor) || me.hasNeighbor(relinkNeighbor)){
                        relinkNeighbor = nodes.get(Config.getRandomNumber(0,nodes.size()-1));
//                        System.out.println("重连时死循环");
//                        if(neighbor.equals(relinkNeighbor)){
//                            relinkNeighbor = nodes.get(Config.getRandomNumber(0,nodes.size()-1));
//                        }
                    }
    ...
    private static int calTotalDegree(ArrayList<Agent> nodes){
        int totalDegree = 0;
...
//        System.out.println(s.hashCode());
    ...
    public static ArrayList<Agent> deepCopy(ArrayList<Agent> agents){
        ...
        for(int i = 0; i < agents.size();i++){
            for(int neighborId:agents.get(i).getNeighborsId()){
                copy.get(i).addNeighbor(getAgentById(copy,neighborId));
                getAgentById(copy,neighborId).addNeighbor(copy.get(i));
            }
        }
    ...
    private static Agent getAgentById(ArrayList<Agent> agents,int id){
