...
import tju.scs.hxt.coordination.symmetric.network.Centrality;
        ...
        ArrayList<ArrayList<Agent>> contrast_experiments = new ArrayList<ArrayList<Agent>>();
            synchronized (getLock(type)){
                if(experiment_agents[type] == null){  // 还未初始化
                    ...
                    // 添加对比试验：1（资源不受限)
                    contrast_experiments.add(1, tju.scs.hxt.coordination.symmetric.network.Network.deepCopy(parameter));

                    // 添加对比试验：2（资源受限，规定资源数量内随机通信)
                    contrast_experiments.add(2, tju.scs.hxt.coordination.symmetric.network.Network.deepCopy(parameter));
