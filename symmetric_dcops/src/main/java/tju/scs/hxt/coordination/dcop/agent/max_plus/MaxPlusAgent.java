 ...
 */
public class MaxPlusAgent extends Agent {
    ...
    // coordination set
    @JsonIgnore
    private Set<MaxPlusAgent> coordinationSet = new HashSet<>();

    // default（init） coordination set
    @JsonIgnore
    private Set<MaxPlusAgent> defaultCoordinationSet = new HashSet<MaxPlusAgent>();

    @JsonIgnore
    private double currentPayoff;

    public MaxPlusAgent(int id, int actionNum, double exploreRate, double learningRate, int type) {
    ...
    public void initCoordinationSet(){
        for(Node node:getNeighbors()){
            coordinationSet.add((MaxPlusAgent)node);
            defaultCoordinationSet.add((MaxPlusAgent)node);
        }
    }
     ...
     * @return
     */
    public boolean sendMessageTo(MaxPlusAgent neighbor,int expId) {
        // 1： 向目标发送信息
     ...
     * @return
     */
    private double calDiffer(MaxPlusAgent neighbor,Message messageOld,Message messageNew){
...
//            System.out.println(this.getId()+ " cal message differ : " + scoreNew + " - " + scoreOld + " = " + differ);
     ...
     * @param reward
     */
    private void updateQ(MaxPlusAgent partner,int action,int partnerAction,double reward){
...
//       learning============================================================================
        ...
        MaxPlusAgent partner = (MaxPlusAgent) getNeighbors().get(randomPartner);
...
//            partner.selectCoordinationSet(expId);
    ...
    private void selectCoordinationSet(int expId) {
        // 1：cal max loss
        double maxLoss = Config.loseRate[expId] * Math.max(Math.abs(potentialExpectedUtility(defaultCoordinationSet)),potentialLossInLockOfCoordination(new HashSet<MaxPlusAgent>()));

        // 2：计算 coordination set
        Set<MaxPlusAgent> tempCoordinationSet = new HashSet<MaxPlusAgent>();
        ...
        Set<MaxPlusAgent> markedCoordinationSet = null;
        // 从最小一个元素开始，逐渐递增遍历
        boolean found = false;
        for(int csNum = 1; csNum < defaultCoordinationSet.size() && !found;csNum++){
            // 依次遍历所有包含 csNum 个元素的集合
            for(Set<MaxPlusAgent> selected:selectSet(defaultCoordinationSet,csNum)){
...
//            System.out.println("use the default coordination set!" + defaultCoordinationSet.size() + " == " + coordinationSet.size());
     ...
     * @return
     */
    private static List<Set<MaxPlusAgent>> selectSet(Set<MaxPlusAgent> range,int csNum) {
        List<Set<MaxPlusAgent>> result = new ArrayList<Set<MaxPlusAgent>>();
        if(csNum == 0){
            result.add(new HashSet<MaxPlusAgent>());  // 一个空解
            return result;
        }

        if(csNum == range.size()){
            result.add(new HashSet<MaxPlusAgent>(range));
            return result;
        }
        ...
        Set<MaxPlusAgent> temp = new HashSet<>();
        List<Set<MaxPlusAgent>> subResult;
        Set<MaxPlusAgent> subRange = new HashSet<>(range);
        for(Iterator<MaxPlusAgent> iterator = range.iterator();iterator.hasNext();){
            MaxPlusAgent currentEle = iterator.next();
            // 1：放当前元素
            temp.add(currentEle);
            subRange.remove(currentEle);
            subResult = selectSet(subRange,csNum-1);
            if(subResult.size() > 0){
                for(Set<MaxPlusAgent> subItem:subResult){
                    subItem.addAll(temp);
                }
     ...
     * @return
     */
    private double potentialLossInLockOfCoordination(Set<MaxPlusAgent> CS){
     ...
     * @return
     */
    private double potentialExpectedUtility(Set<MaxPlusAgent> coordinationSet){
     ...
     * @return
     */
    private double potentialExpectedUtility(Set<MaxPlusAgent> coordinationSet,int actionMe){
        double poe = 0;
        // 1：显示的 coordination set 中的每一个 agent
        for(MaxPlusAgent partner:coordinationSet){
            poe += getMax(qTables.get(partner.getId()).getQ()[actionMe]);
        }

        // 2：隐式的，虽然未在 coordination set，中，但根据统计对手选择action的概率，计算一个 reward 期望
        Set<MaxPlusAgent> others = new HashSet<MaxPlusAgent>();
        for(MaxPlusAgent agent:defaultCoordinationSet){
            if(!coordinationSet.contains(agent)){
                others.add(agent);
            }
        }
        for(MaxPlusAgent partner:others){
     ...
     * @return
     */
    private double calExpectedReward(MaxPlusAgent partner,int actionMe){
        double er = 0;
    ...
    public Set<MaxPlusAgent> getCoordinationSet() {
...
//        System.out.println(Arrays.toString(array));
        ...
        Set<MaxPlusAgent> set = new HashSet<>();
        for(int i = 0; i < 5;i++){
            set.add(new MaxPlusAgent(i,3,0,0,0));
        }
